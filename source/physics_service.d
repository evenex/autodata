...
final class Physics: Service
	{/*...}*/
				...
				debug {/*...}*/
					True_Body.Type type ()
						{/*...}*/
							world.send (id, "type");
							True_Body.Type type;
							receive ((True_Body.Type reply) {type = reply;});
							return type;
						}
				}
			}
		private struct True_Body
			{/*...}*/
				private {/*imports}*/
					import dchip.all;
				}
				...
				private {/*☀}*/
					this (T) (cpSpace* space, T geometry, float mass, vec position, vec velocity, float damping, Body.Id id)
						...
						body {/*...}*/
							this.id = id;
							with (True_Body.Type) {/*deduce shape}*/
								import std.math;
				...
				body {/*...}*/
					...
					Upload upload =
						{/*...}*/
							...
							index: cast(uint)vertices.length,
							length: cast(uint)geometry.length
						};

					static if (is (T: vec[]))
						vertices.data ~= geometry;
					else vertices.data ~= geometry.array;
		...
		private:
		private {/*data}*/
			shared Swap_Buffer!vec vertices;
		}
		static:
		static {/*context}*/
			import dchip.all;
			cpSpace* space;
			True_Body[Body.Id] bodies;
		...
		unittest
			{/*shape deduction}*/
				...
				p.update ();
				assert (a.type == True_Body.Type.polygon);
				assert (b.type == True_Body.Type.polygon);
				assert (c.type == True_Body.Type.circle);
				assert (d.type == True_Body.Type.circle);
				...
				p.update ();
				assert (e.type == True_Body.Type.polygon);
				assert (f.type == True_Body.Type.polygon);
				...
				p.update ();
				assert (tri.type == True_Body.Type.polygon);
				assert (pen.type == True_Body.Type.polygon);
				assert (hex.type == True_Body.Type.circle);
			}
		unittest
			{/*body positioning}*/
		...
		unittest
			{/*box_query}*/
				...
				p.update;
				assert (p.box_query ([vec(-1),vec(1)])		.length == 2);
