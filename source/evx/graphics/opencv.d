...
private {/*imports}*/
	...
	alias floor = evx.analysis.floor; // TODO AnalysisToolkit
	alias ceil = evx.analysis.ceil;
	alias round = evx.analysis.round;
	alias sum = evx.arithmetic.sum; // TODO ArithmeticToolkit
}
...
void umain ()
	{/*...}*/
		auto input_video = Video.Input (`../../Downloads/20141014T18758.avi`);
		auto data_file = readText (`../../Downloads/fcapdata/dat/capture_20141014T184758_vlad.dat`).splitLines;

		{/*process data}*/
			auto plot_size = CvSize (input_video.size.x, input_video.size.y/3);

			scope gfx = new Display (plot_size.x, plot_size.y * 3);
			...
			{/*process data}*/
				...
				{/*make a video}*/
					auto video_duration = 16.seconds;

					auto n_data_points_per_frame = 1;

					auto n_samples = (video_duration * freq).round.to!size_t;

					auto bounds = [gfx.dimensions/vec(1,3), 0.vec].from_pixel_space.to_extended_space (gfx);
					auto Δy = 2*(gfx.dimensions * vec(1, 1/3.)).from_pixel_space.to_extended_space (gfx) * vec(0,1);

					auto plot_img = Image (gfx.dimensions[].map!(to!int).vector!2.tuple.expand);
					{/*generate plot image}*/
						plot (grf[].map!(f => f.z)
							.versus (ℕ[0..grf.length].map!(i => i / freq))
					...
					auto output_video = Video.Output (`out.avi`, CvSize (input_video.size.x, (input_video.size.y + plot_size.y*3))); // BUG if the filepath can't be reached, segfault from opencv
					...
					foreach (i, frame; enumerate (input_video))
						{/*...}*/
							...
							auto plotbox = plot (grf[].map!(f => f.x).versus (ℕ[0..grf.length].map!(to!double))).inside (bounds).using (gfx, scr).plot_field; // TODO
							int x = (i/(n_samples/freq * input_video.framerate) * plotbox.width + (plotbox.left + 1));
