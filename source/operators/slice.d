...
template SliceOps (T...)
	{mixin SubOps!(SourceAccess.by_ref, T);}

template AdaptorOps (T...)
	{mixin SubOps!(SourceAccess.by_value, T);}

enum SourceAccess {by_value, by_ref}

template SubOps (SourceAccess source_access, alias access, LimitsAndExtensions...)
	{/*...}*/
		private {/*imports}*/
				...
				body {/*...}*/
					...
					static if (is_proper_sub || is_trivial_sub)
						{/*...}*/
							...
							alias Subspace = Sub!(
								Select!(
									source_access is SourceAccess.by_ref,
									Source*, Source
								),
								Map!(AxisType,
									Enumerate!(
...
struct Axis (uint axis_index, AxisType)
	{/*...}*/
		...
		else alias Interval = autodata.core.interval.Interval!AxisType;
	}

struct Sub (Source, Axes...)
	{/*...}*/
		mixin SubGenerator!(Source, Filter!(λ!q{(Axis) = Axis.is_free}, Axes))
			sub;
...
template SubGenerator (Source, FreeAxes...)
	{/*...}*/
		mixin LambdaCapture;
				...
				Sort!(λ!q{(T,U) = T.index < U.index},
					Cons!(
						...
						Filter!(λ!q{(Axis) = not (Contains!(Axis.index, Extract!(q{index}, FreeAxes)))},
							Map!(Axis,
								Enumerate!(Domain!(typeof(source.index_ops.opIndex)))
							),
						)
					)
				)
				...
				body {/*...}*/
					auto selection (uint i)()
						{/*...}*/
							...
							auto left_limit ()() {return inf_check (limit!j.left);}
					...
					return source.opIndex (Map!(selection, Ordinal!(Domain!(typeof(source.index_ops.opIndex)))));
				}
		}
		public {/*opSlice}*/
