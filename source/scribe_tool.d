		...
		private {/*ops}*/
			void output (Text order)
				...
				body {/*...}*/
					...
					glyphs[] = text
						.map!(c => glyph (c, size, color));

					cards[] = typeset (glyphs, order, cards);
			...
			auto typeset (T1, T2)(ref T1 glyphs, ref Text order, ref T2 cards)
				...
				body {/*...}*/
					...
					auto card_box = Bounding_Box ([0.vec, vec(0, -font.height)]);
					...
					foreach (i, glyph; glyphs[])
						{/*set card coordinates in pen-space}*/
							vec offset = cast(vec) glyph.offset;
							vec dims   = cast(vec) glyph.dims;

							auto card = cards[4*i..4*(i+1)];
							card[0] = pen;
							card[1] = pen + vec(dims.x, 0);
							card[2] = pen + dims;
							card[3] = pen + vec(0, dims.y);

							if (card[1].x > wrap_width)
								{/*word wrap}*/
							...
							if (card[1].x > card_box.right)
								card_box.right = card[1].x;

							card.map!(v => v - vec(-offset.x, dims.y - offset.y))
								.copy (card);
